...
const useSpeechAssistant = () => {
  ...
  useEffect(() => {
    ...
    recognition.onresult = (event) => {
      ...
      // Get detected language from final result
      const last = event.results.length - 1;
      const lang = event.results[last].isFinal 
        ? (event.results[last][0].language || recognition.lang || null)
        : null;
      ...
      if (lang) {
        ...
        if (langLower.includes('hi') || langLower.includes('hindi')) {
          setSelectedLanguage('hi-IN');
        } else if (langLower.includes('kn') || langLower.includes('kannada')) {
          setSelectedLanguage('kn-IN');
        } else if (langLower.includes('en') || langLower.includes('english')) {
          setSelectedLanguage('en-IN');
        }
      }
      
      // Set transcript - show interim results, but only use final for auto-send
      if (finalTranscript.trim()) {
        ...
        // Show interim results in console but don't set transcript yet
        console.log('Interim transcript:', interimTranscript);
        
        // Clear auto-stop timeout if we get interim results (user is still speaking)
        if (autoStopTimeoutRef.current) {
          clearTimeout(autoStopTimeoutRef.current);
          autoStopTimeoutRef.current = null;
        }
    ...
    recognition.onerror = (event) => {
      ...
      if (event.error === 'no-speech') {
        // Silent timeout - this is normal, don't stop listening
        console.log('No speech detected (normal timeout)');
        // Don't stop listening, just log it
        ...
        setIsListening(false);
        isListeningRef.current = false;
        setSupportError('Network error: Unable to connect to speech recognition service. Please check your internet connection and try again.');
      } else {
    ...
    recognition.onend = () => {
      ...
      // With continuous: true, onend should only fire when manually stopped
      // or on error. Check if we should keep listening
      setIsListening(false);
      isListeningRef.current = false;
