...
import { MessageCircle, Search, Users, Scale, Shield, Brain, FileText, Play, Globe, Bot, Mic, MicOff } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { useEffect, useState, useRef } from 'react';
import { useLanguage } from '../contexts/LanguageContext';
import KannadaVoiceAgent from '../components/KannadaVoiceAgent';
...
function Home() {
  ...
  // DBOSS Feature States
  const [isListening, setIsListening] = useState(false);
  const [showVideo, setShowVideo] = useState(false);
  const [videoStage, setVideoStage] = useState(0); // 0: None, 1: dboss1, 2: dboss2
  const [recognition, setRecognition] = useState(null);
  const videoRef = useRef(null);
  const [debugTranscript, setDebugTranscript] = useState(""); // For debugging

  // Reset debug transcript when listening starts
  useEffect(() => {
    if (isListening) setDebugTranscript("");
  }, [isListening]);
  ...
  useEffect(() => {
    ...
    if ('webkitSpeechRecognition' in window || 'SpeechRecognition' in window) {
      ...
      recognitionInstance.lang = 'en-US'; // Adjust if Kannada/other languages needed for this specifically

      recognitionInstance.onresult = (event) => {
        const transcript = event.results[0][0].transcript.toLowerCase();
        setDebugTranscript(transcript); // Show user what was heard
        console.log('Heard (DBOSS):', transcript);

        // Keywords: "Darshan", "DBoss", "The Boss", "D-Boss"
        const hasDarshan =
          transcript.includes('darshan') ||
          transcript.includes('dboss') ||
          transcript.includes('d boss') ||
          transcript.includes('the boss') || // Common misinterpretation
          transcript.includes('boss') ||
          transcript.includes('updates'); // Fallback for "updates"

        // Trigger condition
        if (hasDarshan) {
          console.log("DBOSS Triggered! Playing sequence...");
          setShowVideo(true);
          setVideoStage(1);
          setIsListening(false); // Stop listening immediately on match
        }
        // If no match, we just keep listening until timeout or user stops
      };
  ...
  // Video playback is now handled via the 'key' prop and 'autoPlay' attribute in the JSX.
  // This ensures the video element is fresh and plays reliably when the stage changes.
  ...
  const handleMicClick = () => {
    if (isListening) {
      ...
      try {
        ...
        // Stop listening after 5 seconds (increased from 3s)
        const timeoutId = setTimeout(() => {
          if (recognition) {
            recognition.stop();
            console.log("Stopped listening after 5 seconds");
          }
        }, 5000);
  ...
  return (
    <div className="min-h-screen relative overflow-hidden">
            ...
            >
              ...
              >
                ...
                >
                  ...
                  <span className="relative z-10">{t('nav.documentTemplates')}</span>
                </Link>
              </motion.div>
              ...
              >
                ...
                >
                  ...
                  <span className="relative z-10">{t('nav.videoTutorials')}</span>
                </Link>
              </motion.div>
            </motion.div>
          </motion.div>
      ...
      <div className="fixed bottom-8 right-8 z-50 flex flex-col items-end gap-4">
        ...
        <div className="relative flex flex-col items-end">
          <AnimatePresence>
            {showLangMenu && (
              <motion.div
                ...
                className="absolute bottom-full mb-4 right-0 bg-gray-900/90 backdrop-blur-xl border border-white/20 rounded-2xl p-2 shadow-2xl overflow-hidden min-w-[160px]"
              >
                {[
                  ...
                  <motion.button
                    ...
                    className={`w-full text-left px-4 py-3 rounded-xl transition-colors text-sm font-medium ${language === lang.code ? 'text-blue-400 bg-blue-500/10' : 'text-white'
                      }`}
                  >
                    {lang.label}
                  </motion.button>
          ...
          <motion.button
            ...
            className="h-16 w-16 rounded-full flex items-center justify-center shadow-[0_0_30px_rgba(100,150,255,0.5)] border border-white/20 relative group overflow-hidden"
            style={{
      ...
      {/* DBOSS Feature Components */}

      {/* Mic Button */}
      <motion.button
        className={`fixed bottom-4 left-4 z-50 p-4 rounded-full shadow-lg transition-all duration-300 ${isListening
        ...
        whileTap={{ scale: 0.9 }}
        style={{
          boxShadow: isListening ? '0 0 15px rgba(220, 38, 38, 0.7)' : '0 0 10px rgba(0,0,0,0.5)'
        }}
      >
        {isListening ? (
          <Mic className="text-white w-6 h-6 animate-bounce" />
        ) : (
      ...
      {isListening && (
        <div className="fixed bottom-20 left-4 z-50 bg-black/80 text-white p-2 rounded text-sm max-w-[200px]">
          Listening... <br />
          {debugTranscript && <span className="text-yellow-400">Heard: {debugTranscript}</span>}
        </div>
      )}

      {/* Video Popup - Sequential */}
      {/* We use a key to force re-mounting or simply update src. Creating a consistent container is better. */}
      {/* Preloading both is safer for 0.2s gap, but switching src is easier to implement. Let's toggle src. */}
      ...
      <AnimatePresence>
        {showVideo && (
          <motion.div
            ...
            className="fixed bottom-20 left-4 z-50 w-72 h-48 bg-black rounded-lg overflow-hidden shadow-2xl border-2 border-red-600"
          >
            <video
              key={videoStage} // Force re-mount on stage change for reliable autoplay
              src={videoStage === 2 ? "/video/dboss2.mp4" : "/video/dboss1.mp4"}
              autoPlay
              playsInline
              ...
              onEnded={() => {
                if (videoStage === 1) {
                  setTimeout(() => setVideoStage(2), 200);
